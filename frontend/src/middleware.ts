...
function applyRefreshCookies(res: NextResponse, r: RefreshResult): void {
  ...
  // httpOnly MUST stay false: the client-side `api.ts` attaches the Bearer
  // header by reading `wt_access` via `document.cookie`, which cannot see
  // HttpOnly cookies. Setting httpOnly here silently broke every SWR fetch
  // after the first automatic refresh — the new token was present for
  // middleware but invisible to the page, so the UI kept sending the
  // expired one and 401'd everywhere.
