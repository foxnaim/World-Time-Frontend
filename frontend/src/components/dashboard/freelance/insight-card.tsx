    ...
    <Card className={cn('relative overflow-hidden p-0', 'border-stone/20', className)}>
      ...
      <div className="flex flex-col gap-6 p-6 pl-8 md:flex-row md:items-center md:p-8 md:pl-10">
        <div className="flex-1">
          ...
          <h3
            className="mt-2 text-2xl font-medium tracking-editorial text-[#3d3b38] md:text-3xl"
            style={{ fontFamily: 'Fraunces, serif' }}
          >
            {rate != null ? `${Math.round(rate)} ₽/час` : 'Пока нет данных'}
          </h3>
          <p className="mt-3 text-sm leading-relaxed text-[#3d3b38] md:text-base">{insight}</p>
          {rate != null && (
            <div className="mt-5">
              <RateScale rate={rate} />
            </div>
