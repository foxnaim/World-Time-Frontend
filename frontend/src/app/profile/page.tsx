...
import { GoogleConnect } from '@/components/profile/google-connect';
...
function displayName(me: Me, fallback: string): string {
...
async function fetchMe(token: string): Promise<Me | null> {
  // API_BASE may or may not already end in /api (NEXT_PUBLIC_API_URL has it,
  // bare API_URL typically doesn't). Normalize: strip trailing /api so we can
  // append a single /api/... path below without double-prefixing.
  const trimmed = API_BASE.replace(/\/+$/, '').replace(/\/api$/, '');
  const url = `${trimmed}/api/auth/me`;
  try {
...
export default async function ProfilePage() {
  ...
  const name = displayName(me, t('profile.account'));

  return (
    <div className="min-h-screen bg-[#EAE7DC] text-[#3d3b38] px-8 py-12 md:px-16 md:py-16">
      <div className="max-w-3xl mx-auto flex flex-col gap-10">
        <header className="flex items-center justify-between gap-6">
          <div>
            <div className="text-[10px] uppercase tracking-[0.28em] text-[#6b6966] mb-2">
              {t('profile.sectionTitle')}
            </div>
            <h1
              className="text-3xl tracking-tight text-[#3d3b38]"
              style={{ fontFamily: 'Fraunces, serif' }}
            >
              {name}
            </h1>
            {me.username && (
              <div className="text-sm text-[#6b6966] mt-1">@{me.username}</div>
            )}
          </div>
          <Link
            href="/dashboard"
        ...
        <section className="border border-[#8E8D8A]/20 rounded-xl p-6 flex flex-col gap-3 bg-[#EAE7DC]">
          ...
          <dl className="grid grid-cols-[140px_1fr] gap-y-2 text-sm">
            <dt className="text-[#6b6966]">{t('profile.name')}</dt>
            <dd>{name}</dd>
            <dt className="text-[#6b6966]">{t('profile.username')}</dt>
            <dd>{me.username ? `@${me.username}` : '—'}</dd>
            <dt className="text-[#6b6966]">{t('profile.telegramId')}</dt>
            <dd className="font-mono">{maskTelegramId(me.telegramId)}</dd>
            <dt className="text-[#6b6966]">{t('profile.email')}</dt>
            <dd>{me.email ?? '—'}</dd>
          </dl>
        </section>
        ...
        <section className="border border-[#8E8D8A]/20 rounded-xl p-6 flex flex-col gap-3 bg-[#EAE7DC]">
          <div className="text-[10px] uppercase tracking-[0.28em] text-[#6b6966]">
            {t('profile.googleAccountSection')}
          </div>
          <GoogleConnect />
        </section>
        ...
        <section className="border border-[#8E8D8A]/20 rounded-xl p-6 flex flex-col gap-3 bg-[#EAE7DC]">
          ...
          {me.employees.length === 0 ? (
            <p className="text-sm text-[#6b6966]">{t('profile.noCompanies')}</p>
          ) : (
                  ...
                  >
                    <div className="min-w-0">
                      <Link
                        ...
                        className="text-sm tracking-tight text-[#3d3b38] hover:text-[#E98074] transition-colors truncate block"
                        style={{ fontFamily: 'Fraunces, serif' }}
                      >
                        {emp.company.name}
                      </Link>
                      <div className="text-[10px] uppercase tracking-[0.22em] text-[#6b6966] mt-0.5">
        ...
        <section className="border border-[#E85A4F]/30 rounded-xl p-6 flex flex-col gap-3 bg-[#EAE7DC]">
