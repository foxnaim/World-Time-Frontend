...
function OnboardingCompanyView() {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      // Coords are optional — only send when the user actually typed them.
      // Zero is a legal lat/lng (Gulf of Guinea) but also the form default,
      // so treat {0,0} as "not set" to avoid spurious geofence writes.
      const hasCoords = form.lat !== 0 || form.lng !== 0;
      ...
      if (hasCoords) {
        payload.latitude = form.lat;
        payload.longitude = form.lng;
        payload.geofenceRadiusM = form.geofenceRadius;
      }
      const res = await api.post<CompanyCreateResponse>('/companies', payload);
  ...
  return (
    ...
    >
      <div className="mb-8 text-center">
        <p className="text-[10px] uppercase tracking-[0.28em] text-[#6b6966] mb-3">
          Онбординг · Компания
        </p>
        <h1
          className="text-3xl md:text-4xl font-medium tracking-tight text-[#3d3b38]"
          ...
          >
            ...
            {step === 1 && (
              <div className="space-y-5">
                <FormField
                  ...
                  autoFocus
                  hint="Нужен только для подписи QR-скрина и отчётов"
                />
                <details className="border-t border-stone/15 pt-4">
                  <summary className="cursor-pointer text-xs uppercase tracking-[0.22em] text-[#3d3b38] hover:text-coral select-none">
                    Настроить геозону (не обязательно)
                  </summary>
                  <p className="mt-3 text-xs text-[#6b6966] leading-relaxed">
                    Если задашь координаты и радиус — бот проверит, что сотрудник физически рядом с
                    офисом при чекине. Можно пропустить и настроить позже в разделе «Настройки».
                  </p>
            ...
            {step === 2 && (
              <div className="space-y-5">
                ...
                <div className="flex flex-col gap-1.5">
                  <label
                    htmlFor="tz"
                    className="text-[10px] uppercase tracking-[0.22em] text-[#3d3b38]"
                  >
                    Часовой пояс
                  </label>
                  <select
                    ...
                    className="h-10 w-full bg-transparent border-0 border-b border-stone/40 px-0 py-2 text-[#3d3b38] focus:outline-none focus:border-coral transition-colors duration-200 rounded-none"
                  >
                    {TIMEZONES.map((tz) => (
            ...
            {step === 3 && (
              <div className="space-y-4">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.22em] text-[#6b6966] mb-3">
                    Проверьте данные
                  </p>
...
const ReviewRow: React.FC<{ label: string; value: React.ReactNode }> = ({ label, value }) => (
  <div className="flex items-baseline justify-between gap-4 py-2.5">
    <dt className="text-[10px] uppercase tracking-[0.22em] text-[#6b6966] whitespace-nowrap">
      {label}
    </dt>
    <dd
      className="text-sm text-[#3d3b38] font-medium text-right"
      style={{ fontFamily: 'Fraunces, serif' }}
    >
      {value}
    </dd>
