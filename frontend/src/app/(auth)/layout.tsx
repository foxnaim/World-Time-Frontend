...
export default function AuthLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative min-h-screen w-full bg-cream text-[#3d3b38] overflow-hidden">
      ...
      <header className="relative z-10 px-6 py-6 md:px-10 md:py-8">
        <Link
          href="/"
          className="inline-flex items-baseline gap-2 text-[#3d3b38] hover:text-coral transition-colors"
        >
          <span
      ...
      <footer className="relative z-10 px-6 pb-8 text-center">
        <p className="text-[10px] uppercase tracking-[0.28em] text-stone/50">
          © {new Date().getFullYear()} Work Tact
        </p>
      </footer>
    </div>
  );
